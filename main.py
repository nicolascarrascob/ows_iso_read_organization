...
def search_org(filepath, input_file_path, output_file_path):
    ...
    for line in xml_files:
        ...
        try:
            ...
            print("error")
            print(str(line.strip()) + ',error')
    file_output.close()


if __name__ == '__main__':
    xml_file_path = '%s/xml' % RESOURCES
    input_file_path = '%s/gpt_resources_approved_migrar_NO.txt' % RESOURCES
    output_file_path = '%s/gpt_resources_approved_migrar_NO_output.txt' % RESOURCES
    search_org(xml_file_path, input_file_path, output_file_path)

    input_file_path = '%s/gpt_resources_migrar_NO.txt' % RESOURCES
    output_file_path = '%s/gpt_resources_migrar_NO_output.txt' % RESOURCES
    search_org(xml_file_path, input_file_path, output_file_path)

    input_file_path = '%s/gpt_resources_sin_owner.txt' % RESOURCES
    output_file_path = '%s/gpt_resources_sin_owner_output.txt' % RESOURCES
    search_org(xml_file_path, input_file_path, output_file_path)